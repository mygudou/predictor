...
def main():
    ...
    # 定义模型
    model = NBEATSModel(input_dim=1, forecast_horizon=120, stack_layers=3, layer_width=256)
    ...
    predictions = []
    with torch.no_grad():
        current_input = torch.tensor(X_test[0], dtype=torch.float32).reshape(1, -1).to(device)
        for _ in range(len(y_test)):
            pred = model(current_input)  # 预测 (1, forecast_horizon)
            predictions.append(pred[:, 0].item())  # 仅取预测的第一个值

            # 更新当前输入
            pred_array = torch.tensor([[pred[:, 0].item()]], dtype=torch.float32).to(device)
    ...
    future_predictions = []
    with torch.no_grad():
        for _ in range(120 // model.forecast_horizon):  # 根据 horizon 切分预测
            pred = model(current_input)  # (1, forecast_horizon)
            future_predictions.extend(pred.squeeze(0).cpu().numpy())  # 展开预测值

            # 更新当前输入
            pred_array = torch.tensor(pred.squeeze(0).cpu().numpy(), dtype=torch.float32).to(device)
            current_input = torch.cat((current_input[:, model.forecast_horizon:], pred_array.unsqueeze(0)), dim=1)

    # 恢复未来预测值
    future_predictions = np.array(future_predictions[:120]).reshape(-1, 1)

...
def predict_future(model, scaler, initial_input, future_steps, device='cpu'):
    ...
    for _ in range(future_steps):
        ...
        # 构造预测值的特征向量
        pred_feature = np.zeros((1, 1, current_input.shape[2]))  # Shape: [1, 1, feature_dim]
        pred_feature[0, 0, 0] = pred  # 将预测值填充到第一维

        # 更新输入，移除最旧时间步，添加新预测
        current_input = np.append(current_input[:, 1:, :], pred_feature, axis=1)
